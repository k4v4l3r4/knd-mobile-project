...
import { 
  ...
  ShoppingCart, Settings, LogOut, X, ChevronRight, LogIn, Gift, Loader2, Cctv, CreditCard, Network, Store
} from 'lucide-react';
...
export default function Sidebar({ isOpen, setIsOpen }: { isOpen?: boolean; setIsOpen?: (v: boolean) => void }) {
  ...
  const displayedMenus = useMemo(() => {
    ...
    // Add "Toko Saya" for roles that can manage a store (RT, ADMIN_RT, WARGA)
    if (['RT', 'ADMIN_RT', 'WARGA', 'rt', 'admin_rt', 'warga'].includes(user?.role || '')) {
       currentMenus.push({ name: 'Toko Saya', icon: Store, href: '/dashboard/umkm/toko' });
    }

...
import { Search, ShieldCheck, Clock, XCircle, Trash2, RotateCcw } from 'lucide-react';
...
export default function TenantBillingPage() {
  ...
  const [tenantToReset, setTenantToReset] = useState<TenantBilling | null>(null);
  ...
  const confirmReset = (tenant: TenantBilling) => {
  ...
  const handleReset = async () => {
    ...
    if (isDemo) {
      toast.error('Mode Demo: Tidak dapat mereset trial');
      return;
    }

    setIsResetting(true);
    try {
      await SuperAdminService.resetTrial(tenantToReset.id);
      toast.success('Trial berhasil diperpanjang 7 hari');
      fetchTenants();
      setIsResetModalOpen(false);
    } catch (error: any) {
      console.error('Failed to reset trial:', error);
      toast.error(error.response?.data?.message || 'Gagal mereset trial');
    } finally {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {loading ? (
                <tr><td colSpan={10} className="px-6 py-8 text-center text-slate-500">Memuat data...</td></tr>
              ) : tenants.length === 0 ? (
                <tr><td colSpan={10} className="px-6 py-8 text-center text-slate-500">Tidak ada data ditemukan</td></tr>
              ) : (
                tenants.map((tenant) => (
                  <tr key={tenant.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-sm font-medium text-slate-800 dark:text-slate-200">{formatPhoneNumber(tenant.contact_phone)}</span>
                        <span className="text-xs text-slate-500 dark:text-slate-400">{tenant.contact_email}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {tenant.status === 'EXPIRED' && (
                          <button
                            type="button"
                            onClick={() => confirmReset(tenant)}
                            className="inline-flex items-center justify-center px-3 py-1.5 rounded-lg border border-blue-200 dark:border-blue-800 text-blue-600 dark:text-blue-400 text-xs font-semibold hover:bg-blue-50 dark:hover:bg-blue-900/20 transition-colors"
                            title="Reset Trial 7 Hari"
                          >
      ...
      {isResetModalOpen && tenantToReset && (
        <div className="fixed inset-0 bg-slate-900/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] max-w-sm w-full p-8 shadow-2xl scale-100 animate-in zoom-in-95 duration-200 border border-transparent dark:border-slate-800">
            <div className="flex items-center justify-center w-20 h-20 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full mx-auto mb-6">
              <RotateCcw className="w-10 h-10" />
            </div>
            <h3 className="text-xl font-bold text-center text-slate-800 dark:text-white mb-2">Reset Trial 7 Hari?</h3>
            <p className="text-center text-slate-500 dark:text-slate-400 mb-8 leading-relaxed text-sm">
              Apakah Anda yakin ingin memperpanjang trial untuk tenant{' '}
              <span className="font-bold text-slate-800 dark:text-white">
                {tenantToReset.tenant_name}
              </span>{' '}
              selama 7 hari ke depan? Status akan berubah menjadi TRIAL.
            </p>
            <div className="flex gap-3">
              <button
                type="button"
                onClick={() => { setIsResetModalOpen(false); setTenantToReset(null); }}
                className="flex-1 py-3.5 px-4 rounded-2xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
              ...
              <button
                type="button"
                onClick={handleReset}
                className="flex-1 py-3.5 px-4 rounded-2xl bg-blue-500 text-white font-semibold hover:bg-blue-600 shadow-lg shadow-blue-500/20 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                disabled={isResetting}
              >
                {isResetting ? <Clock className="w-4 h-4 animate-spin" /> : <RotateCcw className="w-4 h-4" />}
                {isResetting ? 'Memproses...' : 'Reset Trial'}
              </button>
            </div>
          </div>
        </div>

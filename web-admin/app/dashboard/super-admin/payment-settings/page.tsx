...
export default function PaymentSettingsPage() {
  ...
  const withDefaults = (data: PaymentSettings): PaymentSettings => {
    const defaultPlans: PaymentSettings['plans'] = {
      BASIC_RW_MONTHLY: {
        price: 150000,
        discount_percent: 0,
    ...
    return {
      ...
      plans: {
        BASIC_RW_MONTHLY: {
          ...
          discount_percent:
            data.plans?.BASIC_RW_MONTHLY?.discount_percent ??
            defaultPlans.BASIC_RW_MONTHLY.discount_percent,
          description:
            data.plans?.BASIC_RW_MONTHLY?.description ??
            defaultPlans.BASIC_RW_MONTHLY.description,
          features:
            data.plans?.BASIC_RW_MONTHLY?.features?.length
              ? data.plans.BASIC_RW_MONTHLY.features
              : defaultPlans.BASIC_RW_MONTHLY.features,
        },
        BASIC_RW_YEARLY: {
          price: data.plans?.BASIC_RW_YEARLY?.price ?? defaultPlans.BASIC_RW_YEARLY.price,
          discount_percent:
            data.plans?.BASIC_RW_YEARLY?.discount_percent ??
            defaultPlans.BASIC_RW_YEARLY.discount_percent,
          description:
            data.plans?.BASIC_RW_YEARLY?.description ??
            defaultPlans.BASIC_RW_YEARLY.description,
          features:
            data.plans?.BASIC_RW_YEARLY?.features?.length
              ? data.plans.BASIC_RW_YEARLY.features
              : defaultPlans.BASIC_RW_YEARLY.features,
        },
        LIFETIME_RW: {
          price: data.plans?.LIFETIME_RW?.price ?? defaultPlans.LIFETIME_RW.price,
          discount_percent:
            data.plans?.LIFETIME_RW?.discount_percent ??
            defaultPlans.LIFETIME_RW.discount_percent,
          description:
            data.plans?.LIFETIME_RW?.description ??
            defaultPlans.LIFETIME_RW.description,
          features:
            data.plans?.LIFETIME_RW?.features?.length
              ? data.plans.LIFETIME_RW.features
              : defaultPlans.LIFETIME_RW.features,
        },
      },
    };
  };
    ...
    <div className="space-y-6 pb-20">
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      <Label>Fitur Paket (satu per baris)</Label>
                      <textarea
      ...
      <Card className="border-slate-200 dark:border-slate-800 opacity-90">
        <CardHeader>
          ...
          <CardDescription>
            Centralized: dana ditampung Platform dan dibagi manual. Split: gateway (mis. Xendit)
            membantu otomatisasi bagi hasil dan settlement dengan DANA / e-wallet.
          </CardDescription>
        </CardHeader>
        <CardContent>

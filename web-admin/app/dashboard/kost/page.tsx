...
export default function KostPage() {
  ...
  const handleTenantSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataToSend.append('phone', formatPhoneNumber(tenantFormData.phone));
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        {['ADMIN', 'admin', 'ADMIN_RT', 'admin_rt', 'RT', 'rt', 'WARGA', 'warga', 'WARGA_TETAP', 'warga_tetap'].includes(userRole) && (
          <button
            onClick={() => setIsModalOpen(true)}
            className="bg-emerald-600 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 hover:bg-emerald-700 transition shadow-lg shadow-emerald-600/20"
          >
            <Plus size={18} />
                        ...
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-emerald-100 dark:bg-emerald-900/30 rounded-full flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-bold text-sm border border-emerald-200 dark:border-emerald-800 overflow-hidden relative">
                             {tenant.user.photo_url || tenant.user.avatar ? (
                                <Image 
                                  src={(tenant.user.photo_url || tenant.user.avatar)?.startsWith('http') 
                                    ? (tenant.user.photo_url || tenant.user.avatar) 
                                    : `https://api.afnet.my.id${tenant.user.photo_url || tenant.user.avatar}`} 

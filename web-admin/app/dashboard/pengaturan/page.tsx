...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    if (rt || rw) {
        const parts: string[] = [];
        if (rt) parts.push(`RT ${rt.padStart(3, '0')}`);
        if (rw) parts.push(`RW ${rw.padStart(3, '0')}`);
        setProfile(prev => ({
            ...prev,
            rt_name: parts.join(' ')
        }));
  ...
  const getImageUrl = (path: string | null) => {
    ...
    // Check if path already starts with /storage to avoid double /storage
      ...
      Object.entries(profile).forEach(([key, value]) => {
              if (value !== null && key !== 'logo_url' && key !== 'structure_image_url') {
                if (key === 'contact_phone') {
                  formData.append(key, formatPhoneNumber(value as string));
                } else {
  ...
  const openWalletModal = (wallet?: WalletData) => {
    ...
    if (wallet?.qr_code_url) {
      setQrPreview(getImageUrl(wallet.qr_code_url));
    } else {
      setQrPreview(null);
    }
  ...
  const saveWallet = async (e: React.FormEvent) => {
    ...
    try {
      const formData = new FormData();
      formData.append('name', currentWallet.name);
      formData.append('type', currentWallet.type);
      formData.append('balance', currentWallet.balance.toString());
      if (currentWallet.bank_name) formData.append('bank_name', currentWallet.bank_name);
      if (currentWallet.account_number) formData.append('account_number', currentWallet.account_number);
      ...
      if (currentWallet.id) {
        ...
        await axios.post(`/settings/wallets/${currentWallet.id}`, formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      } else {
        await axios.post('/settings/wallets', formData, {
          headers: { 'Content-Type': 'multipart/form-data' }
        });
      }
  ...
  const saveAdmin = async (e: React.FormEvent) => {
    ...
    try {
      const adminPayload = {
        ...currentAdmin,
        phone: formatPhoneNumber(currentAdmin.phone)
      };

      if (currentAdmin.id) {
        await axios.put(`/settings/admins/${currentAdmin.id}`, adminPayload);
      } else {
        await axios.post('/settings/admins', adminPayload);
      }
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] max-w-md w-full p-8 shadow-2xl scale-100 animate-in zoom-in-95 duration-200 border border-white/20 dark:border-slate-700">
            ...
            <form onSubmit={saveWallet} className="space-y-6">
              ...
              {currentWallet.type === 'CASH' && (
                <div>
                  <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Nomor HP/WA (Untuk Konfirmasi)</label>
                  <input
                    type="text"
        ...
        />
              ...
              {currentWallet.type === 'BANK' && (
                <div className="space-y-6 pt-2 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">QRIS (Opsional)</label>
                    <div 
                      className="group relative border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl p-6 text-center cursor-pointer hover:bg-white dark:hover:bg-slate-800 hover:border-emerald-400 transition-all duration-300 bg-slate-50 dark:bg-slate-800/50"
                      onClick={() => qrInputRef.current?.click()}
                    >
                      {qrPreview ? (
                          <div className="relative">
                            <img src={qrPreview} alt="QRIS Preview" className="mx-auto h-32 object-contain mb-2 drop-shadow-sm group-hover:scale-105 transition-transform" />
                            <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl backdrop-blur-sm">
                              <Pencil className="w-8 h-8 text-white" />
                            </div>
                          </div>
                      ) : (
                          <div className="flex flex-col items-center justify-center py-4 text-slate-400 dark:text-slate-500 group-hover:text-emerald-500 transition-colors">
                              <div className="w-12 h-12 bg-slate-100 dark:bg-slate-700 rounded-full flex items-center justify-center mb-3 group-hover:bg-emerald-50 dark:group-hover:bg-emerald-900/30 transition-colors">

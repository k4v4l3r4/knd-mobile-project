...
import { WargaSkeleton } from '@/components/skeletons/WargaSkeleton';

type SortField = 'name' | 'block' | 'gang';
...
export default function WargaPage() {
  ...
  const handleResetWarga = async () => {
    ...
    const firstConfirm = window.confirm('Yakin ingin menghapus SEMUA data warga di RT ini? Tindakan ini tidak dapat dibatalkan.');
    if (!firstConfirm) return;

    const phrase = prompt('Ketik: HAPUS SEMUA DATA WARGA untuk melanjutkan');
    if (!phrase || phrase.trim().toUpperCase() !== 'HAPUS SEMUA DATA WARGA') {
      toast.error('Reset dibatalkan. Frasa konfirmasi tidak sesuai.');
      return;
    }
  ...
  const sortWargaList = (items: Warga[], field: SortField, direction: SortDirection) => {
    const getKey = (w: Warga) => {
      if (field === 'name') return w.name || '';
      if (field === 'block') return (w.block || '').toString();
      if (field === 'gang') return (w.gang || '').toString();
      return w.name || '';
    };

    return [...items].sort((a, b) => {
      const aKey = getKey(a).toLocaleLowerCase();
      const bKey = getKey(b).toLocaleLowerCase();
      let result = 0;
  ...
  const fetchWargas = async (searchTerm = '', pageParam?: number) => {
    ...
    try {
      ...
      if (isDemo || !token) {
        ...
        setWargas(sortWargaList(paginated as Warga[], sortField, sortDirection));
        setMeta({
      ...
      if (response.data.success) {
        ...
        if (Array.isArray(payload)) {
          setWargas(payload as Warga[]);
          setMeta(null);
        } else {
          setWargas(payload.data as Warga[]);
          setMeta(payload);
  ...
  const handleOpenModal = (mode: 'add' | 'edit', warga?: Warga) => {
    ...
    if (mode === 'edit' && warga) {
      ...
      setFormData({
        ...
        marital_status: warga.marital_status || 'Belum Kawin',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Object.entries(formData).forEach(([key, value]) => {
        if (key !== 'address_ktp') {
            if (key === 'phone') {
                data.append(key, formatPhoneNumber(value as string));
            } else {
      ...
      const addressRt = Number(formData.address_rt || 0);
      const addressRw = Number(formData.address_rw || 0);
      const rtRwText = [
        addressRt > 0 ? `RT ${String(addressRt).padStart(3, '0')}` : null,
        addressRw > 0 ? `RW ${String(addressRw).padStart(3, '0')}` : null
      ].filter(Boolean).join(' / ');
      ...
      if (modalMode === 'add') {
        ...
        toast('Untuk login pertama kali, minta warga gunakan menu "Lupa Password" di aplikasi mobile. Kode dikirim ke WhatsApp.', {
          icon: 'ℹ️',
        });
      } else {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="space-y-8">
        {/* Search Bar - Sticky */}
        <div className="sticky top-0 z-30 py-3 bg-slate-50/80 dark:bg-slate-950/80 backdrop-blur-md shadow-sm border-b border-gray-100 dark:border-gray-800 -mx-4 px-4 sm:-mx-8 sm:px-8 mb-4">
            <div className="relative w-full group max-w-7xl mx-auto">
                <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 group-focus-within:text-emerald-500 dark:group-focus-within:text-emerald-400 transition-colors" size={24} />
                <input
                    ref={searchInputRef}
                    autoFocus
                    type="text"
                    placeholder="Cari nama warga, NIK, atau nomor KK..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                    className="w-full pl-14 pr-6 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800 rounded-xl focus:ring-2 focus:ring-emerald-500/20 dark:focus:ring-emerald-500/10 focus:border-emerald-500 dark:focus:border-emerald-500 outline-none transition-all shadow-sm text-lg text-slate-800 dark:text-white placeholder-gray-400 dark:placeholder-slate-500"
                />
            </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 w-full">
            {/* Left: Sort */}
            <div className="flex items-center gap-3 px-4 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm h-[52px]">
              ...
              <select
                ...
                className="bg-transparent border-none text-sm font-medium text-slate-700 dark:text-slate-100 focus:outline-none focus:ring-0 pr-8 cursor-pointer min-w-[120px]"
              >
                <option value="name">Nama Warga</option>
                <option value="block">Nomor Rumah / Blok</option>
                <option value="gang">Gang</option>
              </select>
              <button
                type="button"
                onClick={() => setSortDirection(prev => (prev === 'asc' ? 'desc' : 'asc'))}
                className="flex items-center gap-2 px-3 py-1.5 text-xs font-bold rounded-full border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-200 bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
                title={sortDirection === 'asc' ? 'Urut A–Z' : 'Urut Z–A'}
              >
            ...
            <button
                ...
                className="flex items-center justify-center gap-2 px-4 py-3 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-800 rounded-2xl font-semibold hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-blue-600 dark:hover:text-blue-400 hover:border-blue-200 dark:hover:border-blue-800 transition-all shadow-sm h-[52px]"
                title="Import CSV"
            >
                <FileSpreadsheet size={20} />
            ...
            <button
                ...
                className="flex items-center justify-center gap-2 px-6 py-3 bg-emerald-600 text-white rounded-2xl font-semibold hover:bg-emerald-700 shadow-lg shadow-emerald-600/20 transition-all hover:-translate-y-0.5 active:scale-95 h-[52px]"
            >

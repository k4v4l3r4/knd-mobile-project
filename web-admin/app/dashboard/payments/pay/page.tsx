...
export default function PaymentPage() {
  ...
  if (invoice.payment_instruction) {
    const instruction = invoice.payment_instruction as any;
    const paymentMode = (invoice as any).payment_mode || instruction.payment_mode || 'CENTRALIZED';
      ...
      <div className="max-w-xl mx-auto py-8 px-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-200 dark:border-slate-800">
          ...
          <div className="p-8">
            ...
            <div className="mb-6 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 p-4">
              <div className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">
                Mode Pembayaran
              </div>
              <div className="text-sm font-bold text-slate-900 dark:text-slate-100 mb-1">
                {paymentMode === 'SPLIT' ? 'SPLIT (Otomatis Bagi Hasil)' : 'CENTRALIZED (Platform)'}
              </div>
              <div className="text-xs text-slate-500 dark:text-slate-400">
                {paymentMode === 'SPLIT'
                  ? 'Gateway akan membantu membagi hasil sesuai konfigurasi split.'
                  : 'Dana diterima Platform dan pembagian hasil dilakukan manual di luar gateway.'}
              </div>
            </div>

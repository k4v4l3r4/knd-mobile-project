...
export default function HeaderSection() {
  ...
  const fetchUnreadCount = async () => {
    try {
      ...
      setHasEmergency(res.data.has_emergency);
    } catch (error) {
      if (!isDemo) {
  ...
  const handleNotificationClick = async (notification: Notification) => {
    if (!notification.is_read) {
      try {
        ...
        // Refresh unread count and emergency status to be accurate
        if (!isDemo && token) {
             fetchUnreadCount();
        }
      } catch (error) {
  ...
  const getIcon = (type: string) => {
    ...
    if (['WARNING', 'EMERGENCY', 'SOS', 'PANIC', 'PANIC_BUTTON'].includes(t)) {
        return <AlertTriangle size={16} className="text-amber-500" />;
    }
    if (t === 'ACTION' || t === 'SUCCESS') {
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            {unreadCount > 0 && (
              <span className={`absolute top-2 right-2 flex h-5 w-5 items-center justify-center rounded-full bg-rose-500 text-[10px] font-bold text-white border-2 border-white dark:border-slate-900 ${hasEmergency ? 'animate-pulse' : ''}`}>
          ...
          {isOpen && (
            <div className="absolute right-0 mt-3 w-80 md:w-96 bg-white dark:bg-slate-900 rounded-xl shadow-xl border border-slate-200 dark:border-slate-800 overflow-hidden z-50 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
              <div className="p-4 border-b border-slate-50 dark:border-slate-800 flex items-center justify-between bg-white dark:bg-slate-900">
                ...
                {unreadCount > 0 && (
                  ...
                  >
                    <Check size={12} strokeWidth={3} /> Tandai Semua Sudah Dibaca
                  </button>
                )}
              </div>
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-slate-800">
                    {notifications.slice(0, 10).map((item) => (
                      <div 
                        ...
                        className={`p-4 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors cursor-pointer flex gap-4 ${!item.is_read ? 'bg-emerald-50/40 dark:bg-emerald-900/10' : ''}`}
                      >
                        <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm ${
                          ['WARNING', 'EMERGENCY', 'SOS', 'PANIC', 'PANIC_BUTTON'].includes(item.type?.toUpperCase()) ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 
          ...
          >
            <div className="h-10 w-10 rounded-full overflow-hidden bg-gradient-to-tr from-emerald-500 to-teal-400 relative ring-2 ring-white dark:ring-slate-800 shadow-lg shadow-emerald-500/30 group-hover:scale-105 transition-transform flex items-center justify-center">
              {user?.avatar || user?.photo_url ? (
                 <img 
                   src={user?.avatar?.startsWith('http') ? user.avatar : `https://api.afnet.my.id${user?.avatar || user?.photo_url}`} 

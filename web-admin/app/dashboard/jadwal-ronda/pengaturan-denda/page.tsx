...
export default function PengaturanDendaPage() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (settingsRes.data.data && settingsRes.data.data.length > 0) {
        const fetched = settingsRes.data.data as FineSetting[];
  ...
  if (!['RT', 'ADMIN_RT', 'ADMIN'].includes(userRole) && userRole !== '') {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 max-w-md">
          Anda tidak memiliki izin untuk mengakses halaman pengaturan denda. Halaman ini hanya untuk pengurus RT.
        </p>
        <button 
          onClick={() => router.push('/dashboard/jadwal-ronda')}
          className="mt-6 px-6 py-2.5 bg-slate-900 dark:bg-slate-700 text-white rounded-xl hover:bg-slate-800 transition-colors font-medium"
        >
          Kembali ke Jadwal Ronda
        </button>
      </div>
    );
  }

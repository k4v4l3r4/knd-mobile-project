...
export default function LaporanDendaPage() {
  ...
  const fetchFines = async () => {
    ...
    try {
      ...
      const [finesRes, userRes] = await Promise.all([
        api.get("/ronda-fines", { params }),
        api.get('/me')
      ]);
      ...
      setFines(finesRes.data.data.data || []);
    } catch (error) {
      if (!isDemo) {
  ...
  const handlePay = async (id: number) => {
    ...
    setProcessingId(id);
    try {
      await api.post(`/ronda-fines/${id}/pay`);
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
              <tr>
                ...
                {['RT', 'ADMIN_RT', 'ADMIN'].includes(userRole) && (
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
              {loading ? (
                <tr>
                  <td colSpan={['RT', 'ADMIN_RT', 'ADMIN'].includes(userRole) ? 6 : 5} className="px-6 py-8 text-center text-slate-500">
                      ...
                      <td className="px-6 py-4 text-right">
                        {fine.status === 'UNPAID' && (
                          <button
                            ...
                            className="inline-flex items-center gap-2 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-medium rounded-lg transition-colors disabled:opacity-50"
                          >

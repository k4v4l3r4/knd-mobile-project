...
Route::middleware(['auth:sanctum', 'role:SUPER_ADMIN'])->prefix('super-admin')->group(function () {
    ...
    Route::post('/tenants/{tenant}/reset-trial', [SuperAdminTenantController::class, 'resetTrial']);
...
Route::middleware(['auth:sanctum', 'tenant.status', 'tenant.feature'])->group(function () {
    ...
    // MOVED OUTSIDE to allow query param token auth (window.open support)
    ...
    Route::delete('/rt/kas/transactions/{id}', [\App\Http\Controllers\Api\KasController::class, 'destroy'])->middleware('permission:kas.create');

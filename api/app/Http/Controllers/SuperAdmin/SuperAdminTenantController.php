...
class SuperAdminTenantController extends Controller
{
    ...
    {
        $query = Tenant::with(['activeSubscription', 'wilayah_rt', 'wilayah_rw', 'adminContact']);
        ...
        $data = $tenants->getCollection()->map(function ($tenant) {
            ...
            // Determine contact phone
            $contactPhone = '-';
            if ($tenant->billing_mode === 'RT' && $tenant->wilayah_rt?->contact_phone) {
                $contactPhone = $tenant->wilayah_rt->contact_phone;
            }
            
            // Fallback to Admin Phone
            if ((!$contactPhone || $contactPhone === '-') && $tenant->adminContact) {
                $contactPhone = $tenant->adminContact->phone;
            }

            // Determine contact email
            $contactEmail = $tenant->adminContact?->email ?? '-';
    ...
    public function resetTrial(Tenant $tenant)
    {
        $tenant->update([
            'status' => 'TRIAL',
        ...
        return response()->json([
            'status' => 'success',
            'message' => 'Masa trial berhasil diperpanjang 7 hari',
            'data' => $tenant
        ]);
    }
}

    ...
    public function plans(Request $request)
    {
        ...
        foreach ($defaultPlans as $code => $planDefaults) {
            ...
            $description = isset($config['description']) && is_string($config['description'])
                ? $config['description']
                : $planDefaults['description'];

            $features = isset($config['features']) && is_array($config['features']) && count($config['features']) > 0
                ? array_values(array_filter($config['features'], fn ($f) => is_string($f) && trim($f) !== ''))

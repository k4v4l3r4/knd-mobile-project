...
class WargaController extends Controller
{
    ...
    {
        $user = $request->user();
        $canViewNik = $user && in_array($user->role, ['ADMIN_RT', 'RT', 'ADMIN_RW', 'SUPER_ADMIN'], true);
        ...
        if ($sortField && in_array($sortField, ['name', 'block', 'gang'])) {
        ...
        if ($request->input('all') === 'true') {
            ...
            $wargas->each(function (User $warga) use ($canViewNik) {
                if ($canViewNik) {
                    $warga->makeVisible('nik');
                } else {
                    $warga->makeHidden('nik');
                }
        ...
        $wargas->getCollection()->each(function (User $warga) use ($canViewNik) {
            if ($canViewNik) {
                $warga->makeVisible('nik');
            } else {
                $warga->makeHidden('nik');
    ...
    public function family(Request $request)
    {
        ...
        $family = User::where('kk_number', $user->kk_number)
          ...
          // Make NIK visible only for admin roles
          $family->each(function ($member) use ($canViewNik) {
              if ($canViewNik) {
                  $member->makeVisible('nik');
              } else {
                  $member->makeHidden('nik');
              }
          });
    ...
    public function store(WargaRequest $request)
    {
        ...
        // Set default values with random initial password (first login via OTP reset)
    ...
    public function import(Request $request)
    {
        ...
        foreach ($data as $index => $row) {
            ...
            'Cerai Mati' => 'Cerai Mati',
                ...
                $maritalStatus = $maritalStatusMap[$maritalStatusRaw] ?? 'Belum Kawin';
    ...
    public function update(WargaRequest $request, string $id)
    {
        ...
        // RBAC Enforcement
        $isAdminOrRt = in_array($user->role, ['ADMIN_RT', 'RT', 'ADMIN_RW', 'SUPER_ADMIN'], true);
        
        if (!$isAdminOrRt) {
            // Non-admin must be KEPALA_KELUARGA to edit
            if ($user->status_in_family !== 'KEPALA_KELUARGA') {
                return response()->json([
                    'success' => false,
                    'message' => 'Hanya Kepala Keluarga yang dapat mengubah data.',
                ], 403);
            }

            // Non-admin can only edit themselves or their family members (same KK)
            if ($warga->kk_number !== $user->kk_number) {
                 return response()->json([
                    'success' => false,
                    'message' => 'Anda hanya dapat mengubah data keluarga anda sendiri.',
                ], 403);
            }
        }
        ...
        // Lock NIK if verified
        if ($warga->data_verified_at && isset($validated['nik']) && $validated['nik'] !== $warga->nik) {
             return response()->json([
                'success' => false,
                'message' => 'NIK tidak dapat diubah karena data sudah diverifikasi.',
            ], 422);
        }
    ...
    public function destroy(Request $request, string $id)
    {
        ...
        if (!$isAdminOrRt) {
            return response()->json([
                'success' => false,
                'message' => 'Penghapusan data hanya dapat dilakukan melalui Admin RT/RW.',
            ], 403);
        }
        ...
        // Anonymize/Clear unique fields to allow reuse and ensure privacy
        $warga->phone = null;
        $warga->email = null;
        $warga->nik = null; 
        $warga->kk_number = null;
        $warga->save();

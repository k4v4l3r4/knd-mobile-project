...
class LetterController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            // Updated validation to check against letter_types table with scoping
            'type' => [
                'required',
    ...
    public function show(string $id)
    {
        ...
        // 2. If user is Admin RT, must be same RT
        if (in_array(strtoupper($user->role), ['ADMIN_RT', 'RT'])) {
            if ($letter->rt_id !== $user->rt_id) {
                return response()->json(['message' => 'Unauthorized Access to other RT'], 403);
            }
            return response()->json([
                'success' => true,
                'data' => $letter
        ...
        // 3. Super Admin / Admin RW (assuming they have wider access, or restrict if needed)
        if (in_array(strtoupper($user->role), ['SUPER_ADMIN', 'ADMIN_RW'])) {
             // For now allow, or add RW check for ADMIN_RW
             return response()->json([
                'success' => true,
                'data' => $letter
            ]);
    ...
    public function update(Request $request, string $id)
    {
        ...
        // Fix: Restrict Admin RT/RT to their own RT
        if (in_array(strtoupper($user->role), ['ADMIN_RT', 'RT']) && $letter->rt_id !== $user->rt_id) {
            return response()->json(['message' => 'Unauthorized Access to other RT'], 403);
        }
    ...
    public function destroy(string $id)
    {
        ...
        // Fix: Restrict Admin RT/RT to their own RT
        if (in_array($role, ['ADMIN_RT', 'RT']) && $letter->rt_id !== $user->rt_id) {
            return response()->json(['message' => 'Unauthorized Access to other RT'], 403);
        }

...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Product::with([
                'user:id,name,phone,photo_url,role,rt_id',
                'store',
                'store.rt:id,city',
            ])
        ...
        return response()->json([
            ...
            'meta' => [
                'umkm_scope' => $scope,
                'umkm_mode' => $mode,
                'umkm_gateway_channel' => $gatewayChannel,
            ],
        ]);
    }
    ...
    {
        $products = Product::with(['user:id,name,phone,photo_url,role,rt_id', 'store', 'store.rt:id,city'])
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'shipping_type' => 'nullable|string|in:PICKUP,LOCAL,COURIER',
            'shipping_fee_flat' => 'nullable|numeric|min:0',
            'variant_note' => 'nullable|string',
        ...
        $data = $request->only([
            ...
            'stock', 'shipping_type', 'shipping_fee_flat', 'variant_note', 'specifications',
        ...
        $product = DB::transaction(function () use ($data, $request) {
            ...
            if ($request->has('variants')) {
                // Ensure variants is array (if sent as JSON string, decode it)
    ...
    public function update(Request $request, Product $product)
    {
        ...
        if (!$isOwner && !$isAdminRT) {
            return response()->json(['message' => 'Unauthorized. Anda hanya bisa mengedit produk milik sendiri atau warga RT Anda.'], 403);
        }

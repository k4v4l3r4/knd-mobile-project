...
class WargaRequest extends FormRequest
{
    protected function prepareForValidation(): void
    {
        ...
        // Map legacy marital status
        if ($this->has('marital_status')) {
            $status = $this->input('marital_status');
            $map = [
                'Lajang' => 'Belum Kawin',
                'LAJANG' => 'Belum Kawin',
            ...
            if (isset($map[$status])) {
                $data['marital_status'] = $map[$status];
            } else {
                 // Try title case fallback
                 $data['marital_status'] = ucwords(strtolower($status));
            }
        }
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'address_rt' => [$requiredOnCreate, 'string', 'max:3'],
            'address_rw' => [$requiredOnCreate, 'string', 'max:3'],

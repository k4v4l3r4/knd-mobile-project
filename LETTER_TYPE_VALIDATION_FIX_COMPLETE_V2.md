...
- Issue: Missing validation for letter type field causing backend rejection
...
User Flow:
1. Open "Layanan Surat"
2. Auto-selects first type ✅
3. Select type from dropdown (or keep default)
4. Fill purpose
5. Tap "Ajukan Surat"
6. ✅ Client validates FIRST:
   If type empty → Shows alert
   If type valid → Submits
7. Submit to API with valid type
8. Success! ✅
...
Find handleSubmit function (around line 141)
...
Inspect Dropdown usage (around line 436):
<Dropdown
  label="Jenis Surat"
  data={letterTypes}
  value={type}

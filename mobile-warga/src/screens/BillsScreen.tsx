  ...
  */
  ...
  const handlePayPress = async () => {
    ...
    if (isExpired) {
      ...
      const isAdmin = user && (user.role === 'ADMIN_RT' || user.role === 'RT');
      Alert.alert(
        t('report.accessLimited'), 
        isAdmin ? t('bills.expiredPayment') : t('bills.expiredPaymentWarga')
      );
      return;
    }
      ...
      <View style={styles.historyBody}>
        <View style={{ flex: 1, marginRight: 12 }}>
            ...
            {item.items && item.items.length > 0 && (
              <View style={{ marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: isDarkMode ? 'rgba(255,255,255,0.1)' : '#f1f5f9' }}>
                {item.items.map((subItem, index) => (
                  <View key={index} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                    <Text style={{ fontSize: 12, color: colors.textSecondary, flex: 1 }}>• {subItem.name}</Text>
                    <Text style={{ fontSize: 12, color: colors.textSecondary }}>{formatRupiah(subItem.amount)}</Text>
                  </View>
                ))}
              </View>
            )}

...
export default function InventoryScreen() {
  ...
  const checkRole = async () => {
    try {
      ...
      if (userData) {
        ...
        setIsRT(['RT', 'ADMIN_RT', 'rt', 'admin_rt'].includes(user.role));
      }
    } catch (e) {
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setNewAsset({ ...newAsset, image: result.assets[0] });
    }
  };

  const handleAddAsset = async () => {
    if (isDemo) {
        Alert.alert(t('common.warning'), 'Mode Demo: Simpan aset tidak diizinkan');
        return;
    }
    if (isExpired) {
        Alert.alert(t('common.warning'), 'Akses Terbatas: Silakan perpanjang langganan');
        return;
    }

    if (!newAsset.name || !newAsset.total_quantity) {
      Alert.alert(t('common.error'), 'Nama dan jumlah wajib diisi');
      return;
    }
    ...
    formData.append('total_quantity', newAsset.total_quantity);
    formData.append('condition', newAsset.condition);
    
    if (newAsset.image) {
      const uri = newAsset.image.uri;
    ...
    try {
      ...
      Alert.alert(t('common.success'), 'Aset berhasil ditambahkan');
      setAddModalVisible(false);
      setNewAsset({ name: '', description: '', total_quantity: '', condition: 'BAIK', image: null });
      fetchData();
    } catch (error: any) {
      console.log('Add asset error:', error);
      Alert.alert(t('common.error'), error.response?.data?.message || 'Gagal menambahkan aset');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      >
        ...
        <View style={styles.cardContent}>
          ...
          >
             ...
             <Text style={{ marginLeft: 4, color: colors.primary, fontSize: 12, fontWeight: '600' }}>
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { height: '80%' }]}>
            <View style={styles.modalHeader}>
                <Text style={styles.modalTitle}>Tambah Aset</Text>
                <TouchableOpacity onPress={() => setAddModalVisible(false)} style={{width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: isDarkMode ? '#334155' : '#f1f5f9'}}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              <TouchableOpacity onPress={pickImage} style={styles.imagePicker}>
                {newAsset.image ? (
                   <Image source={{ uri: newAsset.image.uri }} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
                ) : (
                   <View style={{ alignItems: 'center', padding: 20 }}>
                      <Ionicons name="camera-outline" size={40} color={colors.textSecondary} />
                      <Text style={{ color: colors.textSecondary, marginTop: 8 }}>Pilih Gambar</Text>
                   </View>
                )}
              </TouchableOpacity>

              <Text style={styles.inputLabel}>Nama Aset</Text>
              <TextInput
                style={styles.input}
                value={newAsset.name}
              ...
              <Text style={styles.inputLabel}>Jumlah Total</Text>
              <TextInput
                style={styles.input}
                value={newAsset.total_quantity}
                onChangeText={(text) => setNewAsset({...newAsset, total_quantity: text.replace(/[^0-9]/g, '')})}
                placeholder="0"
              ...
              <View style={{ flexDirection: 'row', gap: 10, marginBottom: 16 }}>
                 <TouchableOpacity 
                   style={[styles.conditionBtn, newAsset.condition === 'BAIK' && styles.conditionBtnActive]}
                   onPress={() => setNewAsset({...newAsset, condition: 'BAIK'})}
                 >
                    <Text style={[styles.conditionText, newAsset.condition === 'BAIK' && styles.conditionTextActive]}>BAIK</Text>
                 </TouchableOpacity>
                 <TouchableOpacity 
                   style={[styles.conditionBtn, newAsset.condition === 'RUSAK' && styles.conditionBtnActive]}
                   onPress={() => setNewAsset({...newAsset, condition: 'RUSAK'})}
                 >
                    <Text style={[styles.conditionText, newAsset.condition === 'RUSAK' && styles.conditionTextActive]}>RUSAK</Text>
                 </TouchableOpacity>
              </View>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
                 <Image 
                  ...
                  style={{ 
                    ...
                    marginBottom: 20, 
                    resizeMode: 'contain',
                    backgroundColor: isDarkMode ? '#1e293b' : '#f8fafc' // bg-slate-800 or bg-slate-50
                  }} 
                />

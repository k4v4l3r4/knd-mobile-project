...
const NotificationsScreen: React.FC<Props> = ({ onNavigate }) => {
  ...
  const handleMarkAllRead = async () => {
    ...
    Alert.alert(
      ...
      t('notifications.confirmMarkAllRead'),
      [
    ...
    return (
      <TouchableOpacity
        style={[
          ...
          isEmergency && { borderColor: '#ef4444', borderWidth: 1, backgroundColor: isDarkMode ? '#450a0a' : '#fef2f2' }
        ]}
        activeOpacity={0.8}
        onPress={() => handlePressItem(item)}
      >
        <View style={[styles.iconContainer, isEmergency && { backgroundColor: '#fee2e2' }]}>
          ...
          <View style={styles.cardHeader}>
            <Text
              style={[
                ...
                isEmergency && { color: '#ef4444', fontWeight: 'bold' }
              ]}
              numberOfLines={1}
            >
              {item.title}
        ...
        {!item.is_read && !isEmergency && (
          <View style={styles.unreadDot} />
        )}
        {isEmergency && !item.is_read && (
